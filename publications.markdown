...
Since machine learning/AI is a best seller right now, we will start with my machine learning contributions:
- [Measuring What AI Systems Might Do: Towards A Measurement Science in AI (2026)](https://arxiv.org/pdf/2603.00063). *Under review*. Situated at the delightful intersection of psychometrics, metaphysics, and machine learning, here we argue that AI capabilities and propensities are dispositional properties, and require a fundamentally causal analysis if they are to be measured robustly.
- [AI alignment is a human problem (2026)](https://osf.io/preprints/psyarxiv/zqngj_v1). *Merely preprinted*. In this highly opinionated primer, we argue that AI alignment is fundamentally a human problem. We need to amplify human supervisory signals across both data volume and capability gaps if we are to truly solve alignment.
- [Can vision language models learn intuitive physics from interaction? (2026)](https://arxiv.org/pdf/2602.06033). *Accepted at ICML*. You'd have thought that giving a vision-language model access to a real physical world would help them learn a thing or two about it. Turns out it doesn't, they don't learn any sort of generalisable physical intuitions from learning to build towers with reinforcement learning.
- [In-Context Function Learning in Large Language Models (2026)](https://arxiv.org/pdf/2602.11863). *Accepted at AISTATS*. We find that Large Language Models can do non-linear Gaussian Process regression in context, and that they have rough priors in low-dimensions and smooth priors in higher dimensions. Moreover, we can steer those priors using Parameter Efficient Post-Training.
